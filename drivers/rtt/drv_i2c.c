...
/* I2C flag wait timeout macros */
#define I2C_WAIT_FLAG_SET_TIMEOUT(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(!i2c_flag_get(periph, flag) && timeout--)
        { \
            if (timeout == 0)
            { \
                rt_kprintf("[I2C] Timeout waiting for flag %s (periph=0x%08x)\n", flag_name, periph); \
                return -RT_ETIMEOUT; \
            } \
        } \
    } while(0)

#define I2C_WAIT_FLAG_RESET_TIMEOUT(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(i2c_flag_get(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_FLAG_GD_SET_TIMEOUT(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(!i2c_flag_get_gd(periph, flag) && timeout--)
        { \
            if (timeout == 0)
            { \
                rt_kprintf("[I2C] Timeout waiting for GD flag %s (periph=0x%08x)\n", flag_name, periph); \
                return -RT_ETIMEOUT; \
            } \
        } \
    } while(0)

#define I2C_WAIT_FLAG_GD_RESET_TIMEOUT(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(i2c_flag_get_gd(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_CTL_BIT_SET_TIMEOUT(periph, bit_mask, bit_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(!(I2C_CTL0(periph) & bit_mask) && timeout--)
        { \
            if (timeout == 0)
            { \
                rt_kprintf("[I2C] Timeout waiting for CTL bit %s (periph=0x%08x)\n", bit_name, periph); \
                return -RT_ETIMEOUT; \
            } \
        } \
    } while(0)

#define I2C_WAIT_CTL_BIT_RESET_TIMEOUT(periph, bit_mask, bit_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while((I2C_CTL0(periph) & bit_mask) && timeout--)
        { \
...
/* Special macros with custom return values */
#define I2C_WAIT_FLAG_SET_TIMEOUT_RET0(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(!i2c_flag_get(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_FLAG_GD_SET_TIMEOUT_RET0(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(!i2c_flag_get_gd(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_FLAG_SET_TIMEOUT_RETI(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(!i2c_flag_get(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_FLAG_RESET_TIMEOUT_RETI(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(i2c_flag_get(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_FLAG_GD_RESET_TIMEOUT_RETI(periph, flag, flag_name) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(i2c_flag_get_gd(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_FLAG_GD_SET_TIMEOUT_GOTO(periph, flag, flag_name, label) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while(!i2c_flag_get_gd(periph, flag) && timeout--)
        { \
...
#define I2C_WAIT_CTL_BIT_RESET_TIMEOUT_GOTO(periph, bit_mask, bit_name, label) \
    do { \
        volatile uint32_t timeout = I2C_TIMEOUT_VALUE; \
        while((I2C_CTL0(periph) & bit_mask) && timeout--)
        { \
...
static uint8_t gd32_i2c_read(rt_uint32_t i2c_periph, rt_uint8_t *p_buffer, rt_uint16_t data_byte)
{
    ...
    {
        /* 参考官方 at24cxx.c 的多字节读逻辑
           使用remaining(剩余字节数)而非循环索引,逻辑更清晰

           1字节读: 直接等RBNE读取
           2字节读: 剩余2字节时等BTC+禁ACK(配合ACKPOS_NEXT),然后读2字节
           3+字节读: 剩余3字节时等BTC+禁ACK,然后读完剩余字节
        */

        rt_uint16_t remaining = data_byte;

        while (remaining > 0)
        {
            /* 3+字节读: 当剩余3字节时,等BTC后禁ACK */
            if (data_byte >= 3 && remaining == 3)
            {
                I2C_WAIT_FLAG_SET_TIMEOUT_RET0(i2c_periph, I2C_FLAG_BTC, "BTC");
                i2c_ack_config(i2c_periph, I2C_ACK_DISABLE);
            }

            /* 2字节读: ACK已在ADDSEND后禁用，这里只需按RBNE读取两个字节 */

            /* 等待RBNE并读取一个字节 */
            I2C_WAIT_FLAG_SET_TIMEOUT_RET0(i2c_periph, I2C_FLAG_RBNE, "RBNE");
...
#endif
    {
        ...
        while (data_byte)
        {
            I2C_WAIT_FLAG_GD_SET_TIMEOUT_RET0(i2c_periph, I2C_FLAG_RBNE_GD, "RBNE_GD");
...
static uint8_t gd32_i2c_write(rt_uint32_t i2c_periph, uint8_t *p_buffer, uint16_t data_byte)
{
    ...
    while (data_byte)
    {
...
#endif
        {
            ...
            I2C_WAIT_FLAG_SET_TIMEOUT_RET0(i2c_periph, I2C_FLAG_TBE, "TBE");
                ;

            while (data_byte)
            {
                /* wait until the TI bit is set */
                I2C_WAIT_FLAG_SET_TIMEOUT_RET0(i2c_periph, I2C_FLAG_TI_GD, "TI_GD");
                /* data transmission */
                i2c_data_transmit(i2c_periph, *p_buffer);
                /* point to the next byte to be written */
                p_buffer++;
...
static rt_ssize_t gd32_i2c_master_xfer(struct rt_i2c_bus_device *bus, struct rt_i2c_msg msgs[], rt_uint32_t num)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (!(msg->flags & RT_I2C_NO_START))
        {
            ...
            if (IS_I2C_LEGACY(gd32_i2c->i2c_periph))
            {
                if (msg->flags & RT_I2C_RD)
                {
                    ...
                    if (1 == msg->len)
                    {
                        /* 1字节读：禁用ACK（在ADDSEND之前） */
                        i2c_ack_config(gd32_i2c->i2c_periph, I2C_ACK_DISABLE);
                    }
                    else if (msg->len == 2)
                    {
                        /* 2字节读：启用ACK + 配置ACKPOS_NEXT */
                        i2c_ack_config(gd32_i2c->i2c_periph, I2C_ACK_ENABLE);
                        i2c_ackpos_config(gd32_i2c->i2c_periph, I2C_ACKPOS_NEXT);
                    }
                    else
                    {
                        /* 3+字节读：启用ACK（后续在gd32_i2c_read中管理） */
                        i2c_ack_config(gd32_i2c->i2c_periph, I2C_ACK_ENABLE);
                    }

                    /* 对于Repeated START（i > 0）可能不需要等待I2CBSY清空 */
                    if (i == 0)
                    {
                        /* 第一个消息：等待总线空闲 */
                        I2C_WAIT_FLAG_RESET_TIMEOUT_RETI(gd32_i2c->i2c_periph, I2C_FLAG_I2CBSY, "I2CBSY");
                    }
                    /* 否则总线可能仍然忙碌（Repeated START情况），直接发送START */

                    /* send the start signal */
                    i2c_start_on_bus(gd32_i2c->i2c_periph);
                    /* i2c master sends START signal successfully */
                    I2C_WAIT_FLAG_SET_TIMEOUT(gd32_i2c->i2c_periph, I2C_FLAG_SBSEND, "SBSEND");

                    /* 发送读地址 */
                    i2c_master_addressing(gd32_i2c->i2c_periph, msg->addr, I2C_RECEIVER);

                    /* 等待ADDSEND标志 */
                    I2C_WAIT_FLAG_SET_TIMEOUT_RETI(gd32_i2c->i2c_periph, I2C_FLAG_ADDSEND, "ADDSEND");
                    i2c_flag_clear(gd32_i2c->i2c_periph, I2C_FLAG_ADDSEND);

                    /* 2字节读的正确时序：ADDSEND后立刻禁用ACK（配合ACKPOS_NEXT） */
                    if (msg->len == 2)
                    {
                        i2c_ack_config(gd32_i2c->i2c_periph, I2C_ACK_DISABLE);
                    }
                ...
                {
                    /* 写操作：如果总线busy先清理 */
                    if (i == 0 && i2c_flag_get(gd32_i2c->i2c_periph, I2C_FLAG_I2CBSY))
                    {
                        /* 第一个消息且总线忙：发送STOP等待空闲 */
                        i2c_stop_on_bus(gd32_i2c->i2c_periph);
                        I2C_WAIT_FLAG_RESET_TIMEOUT_RETI(gd32_i2c->i2c_periph, I2C_FLAG_I2CBSY, "I2CBSY");
                    }
                    /* 否则可能是Repeated START，总线可能仍忙 */
...
#endif
            {
                ...
                else
                {
                    ...
                    I2C_WAIT_FLAG_GD_RESET_TIMEOUT_RETI(gd32_i2c->i2c_periph, I2C_FLAG_I2CBSY_GD, "I2CBSY_GD");
        ...
        if (!IS_I2C_LEGACY(gd32_i2c->i2c_periph))
        {
            if (r_total_byte != 0)
            {
                I2C_WAIT_FLAG_GD_SET_TIMEOUT_GOTO(gd32_i2c->i2c_periph, I2C_FLAG_TC_GD, "TC_GD", out);
            }
        }
#endif
    }
...
out:
    ...
    if (IS_I2C_LEGACY(gd32_i2c->i2c_periph))
    {
        ...
        {
            /* 检查是否还有下一条消息（Repeated START 情况） */
            rt_bool_t is_last_msg = (i == (num - 1));
            rt_bool_t next_msg_same_addr = !is_last_msg && (msgs[i + 1].addr == msg->addr);

            if (is_last_msg || !next_msg_same_addr)
            {
                /* 最后一条消息或地址改变：发送STOP */
                if (msg->flags & RT_I2C_RD)
                {
                    ...
                    i2c_ack_config(gd32_i2c->i2c_periph, I2C_ACK_DISABLE);
                    i2c_stop_on_bus(gd32_i2c->i2c_periph);
                    I2C_WAIT_CTL_BIT_RESET_TIMEOUT_GOTO(gd32_i2c->i2c_periph, I2C_CTL0_STOP, "STOP", final_cleanup);
                }
                else
                {
                    /* 写操作：等待BTC后发送STOP */
            ...
            /* 否则下一条消息会发 Repeated START，不发 STOP */
        }
    }
    else
#endif
    {
        ...
        if (!(msg->flags & RT_I2C_NO_STOP))
        {
            I2C_WAIT_FLAG_GD_SET_TIMEOUT_GOTO(gd32_i2c->i2c_periph, I2C_FLAG_TC_GD, "TC_GD", final_cleanup);
            /* send a stop condition to I2C bus */
            i2c_stop_on_bus_gd(gd32_i2c->i2c_periph);
            /* wait until stop condition generate */
            I2C_WAIT_FLAG_GD_SET_TIMEOUT_GOTO(gd32_i2c->i2c_periph, I2C_FLAG_STPDET_GD, "STPDET_GD", final_cleanup);
